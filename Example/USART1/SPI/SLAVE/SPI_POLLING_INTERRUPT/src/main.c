...
#if A96T418_MODE==0
	printf("\n\ra96T428 USART1 SPI SLAVE TEST!!");
#endif	
#if A96T418_MODE==1
	printf("\n\rA96T418 USART1 SPI SLAVE TEST!!");
#endif		
}
...
void Main( void )
{
	...
	USART1_SPI_Initial(SPI2_SLAVE_MODE, 0, spi_msb_lsb, spi_pol, spi_phase, SPI2_TX_RX_MODE, SPI2_SS_HW_ENABLE);
	...
	while (1)
	{
		if(spi_int_polling)
			...
			USART1_SPI_SlaveWithPolling(write_data, read_data, USART_MAX_BUFFER_SIZE);
