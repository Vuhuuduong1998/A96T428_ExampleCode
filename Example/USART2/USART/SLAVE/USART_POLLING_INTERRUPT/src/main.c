...
#if A96T418_MODE==0
	printf("\n\ra96T428 USART2 USART SLAVE TEST!!");
#endif	
...
void Main( void )
{
	...
	/* Set Alternative function  MISO2(P56) / MOSI2(P57) */
	Port_SetAlterFunctionpin(PORT5, PIN6, 0x3);
	Port_SetAlterFunctionpin(PORT5, PIN5, 0x3);
	/* Set Alternative function  SCK2(P55)  */
	Port_SetAlterFunctionpin(PORT5, PIN5, 0x3);
	
	USART2_USART_Initial(SPI2_SLAVE_MODE, 100000, usart_pol, SPI2_TX_RX_MODE, USART2_DATA_8BIT);
	...
	while (1)
	{
		...
		if(usart_int_polling)
			USART2_USART_SlaveWithInterrupt(write_data, read_data, USART_MAX_BUFFER_SIZE);
		else 
			USART2_USART_SlaveWithPolling(write_data, read_data, USART_MAX_BUFFER_SIZE);
		if(1) { 
			printf("\n\r RX : ");
			for(i=0; i<USART_MAX_BUFFER_SIZE; i++) 
