...
#if A96T418_MODE==0
	printf("\n\ra96T428 USART2 USART MASTER TEST!!");
#endif	
}
...
void Main( void )
{
	...
	USART2_USART_Initial(SPI2_MASTER_MODE, 100000, usart_pol, SPI2_TX_RX_MODE, USART2_DATA_8BIT);
   ...
   NOP_10us_Delay(100);    // 1msec delay
	...
	while (1)
	{
		...
		if(usart_int_polling)
			USART2_USART_SendReceiveDataWithInterrupt(write_data, read_data, USART_MAX_BUFFER_SIZE);
		else 
			USART2_USART_SendReceiveDataWithPolling(write_data, read_data, USART_MAX_BUFFER_SIZE);
