...
#if A96T418_MODE==0
	printf("\n\ra96T428 USART2 SPI MASTER TEST!!");
#endif	
#if A96T418_MODE==1
	printf("\n\rA96T418 USART2 SPI MASTER TEST!!");
#endif	
}
...
void Main( void )
{
	...
	USART2_SPI_Initial(SPI2_MASTER_MODE, 100000, spi_msb_lsb, spi_pol, spi_phase, SPI2_TX_RX_MODE, spi_ss_hw);
   ...
   NOP_10us_Delay(100);    // 1msec delay
	...
	while (1)
	{
		...
		if(spi_int_polling)
			USART2_SPI_SendReceiveDataWithInterrupt(write_data, read_data, USART_MAX_BUFFER_SIZE);
		else 
