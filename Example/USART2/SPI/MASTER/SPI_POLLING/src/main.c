...
void Display_project(void)
{
	...
	printf("\n\ra96T428 USART SPI MASTER POLLING TEST!!");
}
...
void Main( void )
{
	...
	if(test_mode==0)
	{
		USART2_SPI_Initial(SPI2_MASTER_MODE, 100000, SPI2_LSB, SPI2_CPOL_LOW, SPI2_CPHA_1EDGE, SPI2_TX_RX_MODE, SPI2_SS_HW_ENABLE);
		printf("\n\r SPI_SS_HW_ENABLE mode ");
	}
	else if(test_mode==1)
	{
		USART2_SPI_Initial(SPI2_MASTER_MODE, 100000, SPI2_LSB, SPI2_CPOL_LOW, SPI2_CPHA_1EDGE, SPI2_TX_RX_MODE, SPI2_SS_HW_DISABLE);
   ...
   NOP_10us_Delay(100);    // 1msec delay
	...
	while (1)
	{
		...
		USART2_SPI_SendReceiveDataWithPolling(write_data, read_data, USART_MAX_BUFFER_SIZE);
		printf("\n\r TX : ");
		for(i=0; i<USART_MAX_BUFFER_SIZE; i++) 
