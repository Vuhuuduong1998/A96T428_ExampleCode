...
void Display_project(void)
{
	...
	printf("\n\ra96T428 USART2 UART POLLING TEST !!");
}

void putstring(char *str)
{
	while(*str != 0)
		USART2_UART_SendDataWithPolling(str++, 1);
}
...
void Main( void )
{
  ...
  /* Port initialize */
	...
	while (1)
	{
		if(USART2_UART_ReceiveCheckWithPolling(&tempr)) 
		{
			tempw=tempr;
			USART2_UART_SendDataWithPolling(&tempw , 1);  
			printf("%c", tempw);
		}
		if(USART2_GetRTOStatus())
		{
