...
void Display_project(void)
{
	...
	printf("\n\ra96T428 USI_UART INTERRUPT TEST!!");
}
...
void Main( void )
{
  ...
  /* Port initialize */
	...
	putstring("a96T428 USI_UART INTERRUPT TEST!!\n");
	
	NOP_10us_Delay(1);         // 10usec	

	while (1)
	{
		temprx = USI_UART_ReceiveDataWithInterrupt(USART_CH0);
		temptx = temprx;
		USI_UART_SendDataWithInterrupt(USART_CH0, temptx);  
		printf("\n\rR,T(%c,%c)",temprx,temptx);
	}
}
...
{	
	extern void USI0_RXInt_Handler_ISR();
	USI0_RXInt_Handler_ISR();
}

void USI0_TXInt_Handler() interrupt USI0_TX_VECT
{
	extern void USI0_TXInt_Handler_ISR();
	USI0_TXInt_Handler_ISR();
}
