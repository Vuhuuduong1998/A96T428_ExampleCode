...
void Display_project(void)
{
	...
	printf("\n\ra96T428 USI_UART POLLING TEST !!");
}

void putstring(char *str)
{
	while(*str != 0)
		USI_UART_SendDataWithPolling(USART_CH0, str++, 1);
}
...
void Main( void )
{
  ...
  /* Port initialize */
	...
	putstring("a96T428 USI_UART POLLING TEST!!\n");
	
	NOP_10us_Delay(1);         // 10usec	

	while (1)
	{
		USI_UART_ReceiveDataWithPolling(USART_CH0, &temprx, 1);
		temptx=temprx;
		USI_UART_SendDataWithPolling(USART_CH0, &temptx, 1);  
