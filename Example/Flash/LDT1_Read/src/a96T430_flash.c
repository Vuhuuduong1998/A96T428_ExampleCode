	...
	if(key == FLASH_KEY)
	{
		...
		{		
			...
			{								
				...
				{
					Size=PAGE_BUFFER_SIZE-(paddr%PAGE_BUFFER_SIZE);
					paddrSize -= Size;
				}
				else
				{									
					...
					{
						Size = SECTOR_BUFFER_SIZE;
						paddrSize -= Size;//SECTOR_BUFFER_SIZE;
					}
#endif				
					else
					{
						Size = PAGE_BUFFER_SIZE;
						paddrSize -= Size;//PAGE_BUFFER_SIZE;
					}
				}
				
				if(bErase)
				{
					ret = Page_Erase_Flash(FLASH_KEY, paddr, Size);
					if(ret)
					{
						_nop_();
						break;
				...
				{
					if(bWrite==1)
						ret = Page_Write_Flash(FLASH_KEY, paddr, Size, buf);
					else if(bWrite==(u8)0xff)  //prewrite 0x00
						ret = Page_Write_Flash(FLASH_KEY, paddr, Size, 0); //prewrite 0x00
...
unsigned char Page_Erase_Flash(unsigned int key, unsigned long sector_address, unsigned int buf_size)
{
	...
	if((sector_address+buf_size)<(unsigned long)target_addr_start)
	{
		ret=0x80; //address range Failure
		return ret;
	}
		...
		if(buf_size && (sector_address>=(unsigned long)target_addr_start) && (sector_address <= (unsigned long)target_addr_end) )
		{
			...
			if((LVICR & 0x20) == 0x00)
			{
				...
				if(User_ID == FLASH_USER_ID)
				{
					...
					if( (sector_address>=(unsigned long)target_addr_start) && (sector_address <= (unsigned long)target_addr_end) )
					{
